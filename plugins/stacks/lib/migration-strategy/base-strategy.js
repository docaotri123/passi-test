...
module.exports = class BaseStrategy {
    ...
    setDependsOn(resource, logicalId, nestedStackName) {
        ...
        if (!this.lambdaStacks[nestedStackName]) {
            this.lambdaStacks[nestedStackName] = new Array(
                this.resourceConcurrency
            ).fill([], 0, this.resourceConcurrency);
        }

        const leastFilled =
            this.lambdaStacks[nestedStackName].reduce(
        ...
        if (this.lambdaStacks[nestedStackName][leastFilled]) {
            const lastIndex =
                this.lambdaStacks[nestedStackName][leastFilled].length - 1;

            dependsOnLogicalId = this.lambdaStacks[nestedStackName][
                leastFilled
            ][lastIndex];
        }
