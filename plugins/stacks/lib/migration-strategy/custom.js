    ...
    try {
        ...
        if (
            ...
            if (plugin.config.custom) {
                throw new Error(
                    `serverless-plugin-split-stacks: custom mapping file ${plugin.config.custom} not found`
                );
            }
            return {};
        }
...
module.exports = class Custom extends BaseStrategy {
    constructor(plugin) {
        ...
        const customStackMapping = loadCustomizations(
            plugin.serverless,
            plugin
        );
    ...
    getDestination(resource, logicalId) {
        ...
        if (typeof this.plugin.constructor.resolveMigration === 'function') {
            const migration = this.plugin.constructor.resolveMigration.call(
                this,
                resource,
                logicalId,
