...
module.exports = class PerFeatureFunction extends BaseStrategy {
    constructor(plugin) {
        super(plugin);

        if (this.isStrategyActive()) {
            this.apiGatewayResourceMap = this.getApiGatewayResourceMap(
                plugin.serverless
            );
            this.lambdaNames = this.getAllNormalizedLambdaNames(
                plugin.serverless
            );
        }
    }
    ...
    getAllNormalizedLambdaNames(serverless) {
        ...
        return Object.keys(serverless.service.functions)
            .map((lambdaName) =>
                this.plugin.provider.naming.getNormalizedFunctionName(
                    lambdaName
                )
            )
            .sort(
                (normalizedName1, normalizedName2) =>
                    normalizedName2.length - normalizedName1.length
            );
    }
    ...
    getApiGatewayResourceMap(serverless) {
        ...
        apiGatewayPlugin.validated.events.forEach(({ functionName, http }) => {
            ...
            const normalizedLambdaName = this.plugin.provider.naming.getNormalizedFunctionName(
                functionName
            );
            ...
            http.path.split('/').forEach((token) => {
                tokens.push(token);
                const _resourceName = this.plugin.provider.naming.getResourceLogicalId(
                    tokens.join('/')
                );
                if (!resourceLambdasMap.has(_resourceName)) {
                    resourceLambdasMap.set(_resourceName, new Set());
                }
                resourceLambdasMap.get(_resourceName).add(normalizedLambdaName);
            });
        });
        ...
        resourceLambdasMap.forEach((normalizedFunctionNames, resourceName) => {
            ...
            resourceMap.set(
                resourceName,
                normalizedFunctionNames.values().next().value
            );
        });
    ...
    getDestination(resource, logicalId) {
        ...
        if (
            ['AWS::ApiGateway::Method', 'AWS::ApiGateway::Resource'].indexOf(
                resource.Type
            ) !== -1
        ) {
    ...
    // overloaded
    isStrategyActive() {
        return this.plugin.config.perFeature;
    }
