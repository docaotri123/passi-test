...
module.exports = {
        ...
        while (stackName === undefined) {
            ...
            if (this.stackHasRoom(stack)) {
                ...
                if (allowSuffix) {
                    ...
                    throw new Error(
                        `Destination stack ${destination} is already full!`
                    );
                }
            }
        }
    ...
    stackHasRoom(stack) {
        ...
        return (
            Object.keys(resources).length < 500 &&
            ...
            if (Array.isArray(currentValue)) {
                ...
                if (Ref && Ref in this.resourcesById) {
                    ...
                    if (typeof Sub === 'string') {
                        ...
                        let varString = Sub;
                        ({ varString, mappings } = varStringToMappings(
                            varString
                        ));
    ...
    reconcile(resourceId, dependencyId, handlers) {
        ...
        if (resourceMigration) {
            if (dependencyMigration) {
                if (
                    resourceMigration.stackName !==
                    dependencyMigration.stackName
                ) {
    ...
    getNestedStackFiles() {
        ...
        return fs
            ...
            .filter((filename) =>
                filename.match(/nested-stack(-[0-9]+)?\.json$/)
            )
            .map((filename) => {
                return {
                    ...
                    createReadStream: () =>
                        fs.createReadStream(
                            path.join(serverlessDirPath, filename)
                        ),
                };
            });
    },
