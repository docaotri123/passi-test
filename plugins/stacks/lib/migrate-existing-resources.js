...
module.exports = function getCurrentState() {
    ...
    return this.getStackSummary(rootStackName)
        ...
        .then((stackSummary) => {
            ...
            const nestedStacks = stackSummary.filter(
                (summary) =>
                    summary.ResourceType === 'AWS::CloudFormation::Stack'
            );
            ...
            return Promise.all(
                nestedStacks.map(
                    throat(5, (stack) => {
                        ...
                        return this.getStackSummary(nestedStackId).then(
                            (nestedStackSummary) => {
                                nestedStackSummary.forEach((nestedSummary) => {
                                    const logicalId =
                                        nestedSummary.LogicalResourceId;
                                    const resource = this.resourcesById[
                                        logicalId
                                    ];
