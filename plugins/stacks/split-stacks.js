...
class ServerlessPluginSplitStacks {
    constructor(serverless, options) {
        if (!semver.satisfies(serverless.version, '>= 1.13')) {
            throw new Error(
                'serverless-plugin-split-stacks requires serverless 1.13 or higher!'
            );
        }
        ...
        this.hooks = {
            'after:aws:package:finalize:mergeCustomProviderResources': this.split.bind(
                this
            ),
    ...
    split() {
        ...
        this.resourcesById = { ...this.rootTemplate.Resources };
                ...
                return Promise.all(
                    _.map(files, (file) => {
                        ...
                        if (deploymentBucketObject) {
                            const encryptionParams = this.getEncryptionParams(
